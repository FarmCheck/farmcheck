...
export function BrandStep({ form, onSetBodyUpdate }) {
  ...
  useEffect(() => {
    form.setFieldsValue({ isHaveBrand });
  }, [isHaveBrand]);
      ...
      >
        ...
        >
          <TextEditor
            ...
            onChange={value => {
              form.setFieldsValue({ brandDescription: value });
            }}
          />
          <Spacing height={42} />
        </CFormItem>

        <CFormItem name="banner" label={<NameText>Hình ảnh banner</NameText>}>
          <CUploadSingle
            valuePropName="file"
            name="banner"
            disabled={!isHaveBrand}
